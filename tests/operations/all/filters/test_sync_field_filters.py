...
def test_all__filter__eq__ok(
    ...
    results = item_sql_query_manager.query_manager.where(name=expected_item_name).all()
...
def test_all__filter__multiple_filters__ok(
    db_session,
    item_sql_query_manager,
):
    expected_item_name = "expected_item_name"
    ...
    query = item_sql_query_manager.query_manager
...
def test_all__filter__in_not_in__ok(
    db_session,
    item_sql_query_manager,
):
    first_number = 1
...
def test_all__filter__in_not_in__dates__ok(
    db_session,
    item_sql_query_manager,
):
...
def test_all__filter__gt_lt_gte_lte__ok(
    db_session,
    item_sql_query_manager,
):
    for i in range(1, 6):
    ...
    for field, expected_item_numbers in (
        ...
        results = item_sql_query_manager.query_manager.where(**{field: 3}).all()
